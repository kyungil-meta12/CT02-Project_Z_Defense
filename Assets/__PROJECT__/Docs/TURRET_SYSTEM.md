...
- Upgrade and evolution cost slots can route clicks through `TurretItemDescriptionOpenButton` to the shared `TurretItemDescriptionPopupUI`.
- `TurretItemDescriptionPopupUI` is a read-only item relation popup. It shows item name, owned count, icon, description, craft outputs that require the current item, evolution target turrets that require the current item through manually connected `Related Turret Definitions`, acquisition inputs required to obtain the current item, item-to-item navigation history, and an Inventory UI shortcut.
- The default relation mode when opening `TurretItemDescriptionPopupUI` from a cost slot is acquisition inputs, labelled `획득에 필요 자원`. This mode includes direct craft inputs from the current item's `ItemsToCreate` and decompose source items found by reverse-checking every item's `ItemsFromDecompose`. Inventory-close restoration keeps the relation mode that was active before Inventory opened.
- Relation slot amount labels in `TurretItemDescriptionPopupUI` are categorical labels, not counts: craft inputs and craft outputs use `제작 필요`, decompose source items use `분해 필요`, and turret evolution targets use `진화 필요`.
- When the Inventory shortcut is used from `TurretItemDescriptionPopupUI`, the popup stores the current item, relation mode, item navigation history, and turret UI restore state before opening `InventoryUI`. The turret UI is cleared while Inventory is open to avoid raycast blocking, then the previous turret UI and item description popup state are restored from `InventoryUI.InventoryClosed`.
- Display-only TMP fields in `TurretItemDescriptionPopupUI`, including item name, owned count, and description, should not block button input. Runtime code disables their `raycastTarget`, and authored UI should keep display-only text raycast targets off by default.
