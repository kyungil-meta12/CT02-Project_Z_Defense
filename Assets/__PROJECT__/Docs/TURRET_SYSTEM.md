...
- `TurretItemDescriptionPopupUI` is a read-only item relation popup. It shows item name, icon, description, craft outputs that require the current item, evolution target turrets that require the current item through manually connected `Related Turret Definitions`, craft inputs required by the current item, item-to-item navigation history, and an Inventory UI shortcut. It does not display owned count text.
