 ...
 *
 * ==========================================================================================
 * 9. 2026-07-02 EvolutionPopup 런타임 컨텍스트 수정 기록
 * ==========================================================================================
 *
 * 오늘 확인/수정한 내용
 * ------------------------------------------------------------------------------------------
 * - EvolutionPopup의 하단 Evolution 버튼은 선택된 후보 인덱스로 진화를 실행하지만,
 *   진화 성공 후 상위 TurretSelectionUIController의 currentContext가 이전 터렛을
 *   계속 들고 있을 수 있는 흐름을 확인했다.
 * - TurretPopupPageUI에 하위 팝업이 변경된 선택 컨텍스트를 상위 컨트롤러에 반영하는
 *   보호 메서드를 추가했다.
 * - TurretSelectionUIController에 하위 팝업에서 전달된 새 선택 컨텍스트를 저장하고
 *   사거리 표시를 새 터렛 기준으로 갱신하는 메서드를 추가했다.
 * - TurretEvolutionPopupUI는 진화 성공 직후 새 터렛 컨텍스트를 상위 컨트롤러에도
 *   반영한다.
 * - 이 수정으로 진화 후 Back을 눌렀을 때 선택 팝업 복귀가 이전 터렛 참조 때문에
 *   선택 해제로 빠지는 위험을 줄였다.
 *
 * ==========================================================================================
 * 10. 다음 작업 시작용 문장
